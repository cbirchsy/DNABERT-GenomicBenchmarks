...
class GenomicBenchmarksDataModule(pl.LightningDataModule):
    ...
    def _tokenize(self, dataset):
        if self.tokenize_for_dnabert:
            return self.tokenizer(
                [kmerize(seq, self.k) for seq in dataset["seq"]],
                **self.tokenizer_kwargs
            )
        else:
    ...
    def _configure_dataset(self, dataset_name):
        df_train = pd.DataFrame(
            data=list(GenomicClfDataset(dataset_name, split="train", version=0)),
            columns=["seq", "label"],
        )
        ds_train = datasets.Dataset.from_pandas(df_train)

        df_test = pd.DataFrame(
            data=list(GenomicClfDataset(dataset_name, split="test", version=0)),
            columns=["seq", "label"],
        )
        ...
        if self.tokenize_for_dnabert:
            ...
            self.max_length = min(
                ...
                max(
                    max([len(seq) for seq in df_train["seq"]]),
                    max([len(seq) for seq in df_test["seq"]]),
                ),
            )
        else:
            self.model_max_length = 1000
    ...
    def setup(self, stage=None):
        if self.pretokenize:
            self.dataset_train = PreTokenizedGenomicBenchmarksDataset(
                self.dataset["train"], padding=self.padding
            )
            self.dataset_test = PreTokenizedGenomicBenchmarksDataset(
                self.dataset["test"], padding=self.padding
            )
        else:
            self.dataset_train = GenomicBenchmarksDataset(
                self.dataset["train"],
                self.tokenizer,
                self.k,
                self.tokenizer_kwargs,
                padding=self.padding,
                tokenize_for_dnabert=self.tokenize_for_dnabert,
            )
            self.dataset_test = GenomicBenchmarksDataset(
                self.dataset["test"],
                self.tokenizer,
                self.k,
                self.tokenizer_kwargs,
    ...
    def val_dataloader(self):
        return self._data_loader(
            self.dataset_test, shuffle=False
        )  # there is no val set
